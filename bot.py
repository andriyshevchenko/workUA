...
class WorkUABot:
    ...
    async def _search_jobs(self, search_config: dict) -> List[JobListing]:
        ...
        self.logger.info(f"🎯 Ціль сканування: {target_jobs} вакансій (x{config.VACANCY_MULTIPLIER} від мети для запасу)")
    ...
    async def _process_jobs(
        ...
        for idx, job in enumerate(jobs, 1):
            if stats['applied'] >= max_applications:
                self.logger.info(f"🎯 Досягнуто мету: {stats['applied']}/{max_applications} відгуків")
                break
